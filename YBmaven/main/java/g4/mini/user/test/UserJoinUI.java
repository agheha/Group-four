...
public class UserJoinUI extends BaseUI {
	...
	public void service() {
		...
		while (true) {
			String userPH = getString("6. 휴대전화 번호(010-0000-0000) : ");
			if (!Pattern.matches("^01(?:0|1|[6-9])-(?:\\d{3}|\\d{4})-\\d{4}$", userPH)) {
				System.out.println("알맞은 형식이 아닙니다.");
				System.out.println("다시 입력해주세요. ");
				continue;
			}
			
			join.setUserPH(userPH);
			break;
		}

		while (true) {
			String userEmail = getString("7. 이메일 : ");
			if(!Pattern.matches("^[_a-zA-Z0-9-\\.]+@[\\.a-zA-Z0-9-]+\\.[a-zA-Z]+$", userEmail)) {
				System.out.println("알맞은 형식이 아닙니다.");
