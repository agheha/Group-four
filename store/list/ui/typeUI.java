...
public class typeUI extends BaseUI{
	private Scanner sc = new Scanner(System.in);
	private MemberDAO dao;

	public typeUI(MemberDAO dao) {
		this.dao = dao;
	}

	public void service() {
		List<Store> list = dao.selectTypeMember();
		System.out.println("-------------------------------");
		System.out.println("타입별로 보기");
		System.out.println("-------------------------------");
		System.out.println("타입    가게번호    가게이름    메인메뉴    가격");
		System.out.println("-------------------------------");
		int index = 1;
		Map<Integer, Store> priceStore = new TreeMap<>(); 
		for (Store b : list) {
			priceStore.put(index, b);
			System.out.printf("%5s %3d %10s %10s %10d원\n", 
					b.getStoType(),
					index++, 
					b.getStoName(), 
					b.getRstMenu(),
		...
		System.out.print("    " + store.getStoType());
		System.out.print("    " + no);
